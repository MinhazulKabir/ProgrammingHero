...
function amamc_do_askmeanythingpeople( $atts ) {
	...
	extract( shortcode_atts(
		array(
			...
			'success_text'		=> __('প্রশ্নটি গ্রহণ করা হয়েছে এবং এটির উত্তর দেওয়া হলে প্রদর্শিত হবে ।','abl_amamc_td'),
			'question_box_title'=> __('প্রশ্নটি নিচে লিখো । তোমার নাম, ঠিকানা লিখতে ভুলো না কিন্তু...*','abl_amamc_td'),
			'placeholder'		=> __('এই বক্সের মধ্যে প্রথমে প্রশ্নটি লিখবে ।  তারপরে তোমার নাম, ঠিকানা লিখবে ।','abl_amamc_td'),
			'test_question'		=> __('What is 7+5= ?','abl_amamc_td'),
			'test_answer'		=> __('12','abl_amamc_td'),
			'ask_button_text'	=> __('লেখা শেষ হলে এখানে *ক্লিক*','abl_amamc_td'),
			'answer_list_title'	=> __('এখন পর্যন্ত উত্তরসমূহ..','abl_amamc_td'),
						...
						<form id="commentform" class="amaform comment-form" action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post">
							...
							if ( $show_test_question == 'true' ) { ?>
								<p class="comment-form-comment">
									...
									<input type="text" name="amatestquestion" id="ama-testquestion" class="ama-testquestion" placeholder="<?php _e('এখানে, 12 লিখুন','abl_amamc_td') ?>" />
